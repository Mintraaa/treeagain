...
// Function to populate the move options dynamically
function populateMoveOptions() {
  const moveTargetSelect = document.getElementById("moveTarget");
  moveTargetSelect.innerHTML = '<option value="">Select destination</option>';
  populateMoveOptionsRecursive(folderStructure, moveTargetSelect);
}

function populateMoveOptionsRecursive(node, select) {
  if (node.children) {
    node.children.forEach((child) => {
      ...
      populateMoveOptionsRecursive(child, select);
    });
  }
}
...
function moveItem() {
  ...
  if (selectedItem && moveTargetSelect.value) {
    const itemToMove = findItem(folderStructure, selectedItem.textContent);
    const destinationFolder = findItem(folderStructure, moveTargetSelect.value);

    if (itemToMove && destinationFolder) {
      // Remove the item from its current parent
      const parentOfItemToMove = findParent(folderStructure, itemToMove);
      parentOfItemToMove.children = parentOfItemToMove.children.filter((child) => child !== itemToMove);

      // Add the item to the destination folder
      destinationFolder.children.push(itemToMove);

      // Update the folder tree display
      updateFolderTree();
      // Repopulate the move options
      populateMoveOptions();
    }
  }
}
...
// Call the populateMoveOptions function when the page loads
